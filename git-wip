...
env='export GIT_HEIGHT="$(git rev-list --count "${GIT_COMMIT:-HEAD}")" ;\
  ...
  export GIT_MESSAGE="$(printf r%s:\ %s "$(($(git rev-list --count ${GIT_COMMIT:-HEAD}) - 1))" "$(echo "$GIT_TREE_HASH" | python3 -c '"'"'print(" ".join(list(filter(bool, (dict(a="alfa", b="bravo", c="charlie", d="delta", e="echo", f="foxtrot", **{"0": "zero", "1": "one", "2": "two", "3": "three", "4": "four", "5": "five", "6": "six", "7": "seven", "8": "eight", "9": "nine"}).get(c, "") for c in __import__("sys").stdin.read().lower())))[:4]))'"'"')")" ;\
  true';
